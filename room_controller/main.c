...
{	
	...
	temperatureMeasured_decimal &= 0x00000001F; // Assignment
	
	// According to equation 2:

	
	if(temperatureMeasured_decimal == 0x00000001F)
	{
	    temperatureMeasured = 31;
	    if(temperatureMeasured_fractional == 0x00000000)
	    {
	...
	{
	    temperatureMeasured = ((float)temperatureMeasured_decimal*1.0)+((float)temperatureMeasured_fractional*0.2);
	}
	...
	if (isHeaterOn && (temperatureDesired <= temperatureMeasured))
	{
	    isCoolerOn = false; // Assignment
	    isHeaterOn = false; // Assignment
	}
